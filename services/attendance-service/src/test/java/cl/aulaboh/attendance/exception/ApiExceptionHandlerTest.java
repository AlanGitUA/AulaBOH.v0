...
class ApiExceptionHandlerTest {
    ...
    void returnsServiceUnavailableWhenCircuitBreakerIsOpen() {
        var response = handler.circuitBreakerOpen(openCircuitBreakerException("attendance"));
    ...
    void returnsBadRequestForBusinessException() {
        var response = handler.business(new BusinessException("La clase ya existe"));

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertBody(response.getBody(), "BusinessException", "La clase ya existe");
    ...
    private void assertErrorMetric(String type, String status) {
        assertThat(meterRegistry.find("aulaboh.api.errors")
                .tags("service", "attendance-service", "type", type, "status", status)
                .counter()).isNotNull();
    }
}
